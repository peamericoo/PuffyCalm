...
const STEP_META: Record<
  CheckoutStep,
  { label: string; short: string; icon: typeof Mail }
> = {
  contact: { label: "Contact", short: "1", icon: Mail },
  shipping: { label: "Ship to", short: "2", icon: MapPin },
  reward: { label: "Bonus", short: "★", icon: Gift },
  pay: { label: "Pay", short: "3", icon: CreditCard },
};

/** User-facing progress (reward is interstitial, not a numbered step). */
...
/**
 * Modal carousel checkout: Contact → Ship → Reward → Pay.
 * One dialog, back anytime, creative reward between ship & pay.
 */
export function CheckoutView() {
  ...
  return (
    <div className={cn(styles.shell, "relative flex items-center justify-center px-3 py-6 sm:px-5 sm:py-10")}>
      <div className={styles.backdrop} aria-hidden />

      {/* Floating order chips behind modal (context, not friction) */}
      <div className="pointer-events-none absolute inset-x-0 top-[max(5.5rem,env(safe-area-inset-top))] z-40 flex justify-center px-4 opacity-90">
        <div className="flex max-w-md flex-wrap items-center justify-center gap-1.5">
          {items.slice(0, 3).map((item) => (
            <span
              key={item.productId}
              className="inline-flex items-center gap-1.5 rounded-full border border-white/50 bg-white/75 py-1 pl-1 pr-2.5 text-[11px] font-medium text-foreground shadow-sm backdrop-blur-md"
            >
          ...
          <span className="rounded-full border border-white/50 bg-white/80 px-2.5 py-1 text-[11px] font-semibold tabular-nums text-brand-deep shadow-sm backdrop-blur-md">
            {miniTotal}
          </span>
        </div>
      </div>
      ...
      <div
        ...
        className={cn(styles.modal, "animate-fade-up")}
      >
        {/* Header */}
        <header className="shrink-0 border-b border-border/50 px-4 pb-3 pt-3.5 sm:px-5 sm:pt-4">
          ...
          <div className="mb-2.5 flex items-center gap-1.5">
            {(["contact", "shipping", "pay"] as const).map((s, i) => {
              const n = i + 1;
              const done = face > n || (face === n && step === "pay" && s === "pay");
              const active = face === n;
              return (
                <span
                  key={s}
          ...
          <div className="mt-3 flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p id={descId} className="text-[11px] font-medium text-muted-foreground">
                {step === "reward"
              ...
              <h1
                id={titleId}
                className="font-display text-[1.35rem] font-semibold tracking-[-0.03em] text-foreground sm:text-[1.5rem]"
              >
        ...
        <div className={styles.viewport}>
          ...
          >
              ...
              <div className="space-y-3.5 pb-2">
                ...
                <div className="rounded-2xl bg-brand-soft/70 px-3.5 py-3 ring-1 ring-brand/10">
                  <p className="flex items-start gap-2 text-[12.5px] leading-snug text-foreground/85">
                    ...
                    Finish the next step and we’ll unlock a little packing bonus
                    for this order.
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3 pb-2">
                ...
                <label className="block space-y-1.5">
                  ...
                  <Input
                    ...
                    className={cn("rounded-2xl", errors.line1 && "border-cta/60")}
                  />
                </label>
                <label className="block space-y-1.5">
                  <span className="text-[12.5px] font-medium text-foreground/80">
                  ...
                  <Input
                    ...
                    className={cn("rounded-2xl", errors.city && "border-cta/60")}
                  />
                </label>
            ...
            >
              ...
              <div className="relative flex flex-col items-center px-1 pb-3 pt-4 text-center">
                <span
                  className={cn(
                    styles.rewardIcon,
                ...
                <h2 className="mt-1 font-display text-[1.35rem] font-semibold tracking-[-0.03em] text-foreground">
                  Calm Packing is on us
                </h2>
                <p className="mt-2 max-w-[18rem] text-[13.5px] leading-relaxed text-muted-foreground">
                  Because you made it this far — your order gets a care note,
                  soft wrap, and priority hand-off to shipping. No code needed.
                </p>
            ...
            <div className={styles.slide} aria-hidden={step !== "pay"}>
              <div className="space-y-3.5 pb-2">
                ...
                {/* Compact order */}
                <div className="rounded-2xl border border-border/60 bg-white px-3 py-2.5">
        ...
        {/* Footer CTAs */}
        <footer className="shrink-0 border-t border-border/50 bg-white/95 px-4 pb-[max(0.9rem,env(safe-area-inset-bottom))] pt-3 sm:px-5">
          {step === "pay" ? (
            <Button
              ref={continueRef}
              type="button"
          ...
          <p className="mt-2.5 text-center text-[11px] leading-snug text-muted-foreground">
            {step === "pay"
              ? "Mock payment · Secured by Stripe · Guest OK"
